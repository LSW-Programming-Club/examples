...
# Customize the Space Invaders game to make a unique twist!
...
while running:
    ...
    # If keystroke is pressed check whether its right or left and move playerX accordingly
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE and not projectile_fired:
                pygame.mixer.Sound('./src/laser.wav').play()
                projectileX = playerX
    ...
    for i in range(num_of_enemies):
        ...
        if collision:
            pygame.mixer.Sound('./src/explosion.wav').play()
            projectileY = 480
            projectile_fired = False
            score_value += 1
        ...
        if enemyY[i] > playerY - 32:
            for j in range(num_of_enemies):
                enemyY[j] = 2000  # Effectively hide enemies
            game_over_text()
            break
